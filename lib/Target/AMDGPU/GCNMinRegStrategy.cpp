...
void GCNMinRegScheduler::bumpPredsPriority(const SUnit *SchedSU, int Priority) {
	...
	DEBUG(
			dbgs() << "Make the predecessors of SU(" << SchedSU->NodeNum
...
std::vector<const SUnit*> GCNMinRegScheduler::schedule(
		ArrayRef<const SUnit*> TopRoots, const ScheduleDAG &DAG) {
...
//    );
		...
		std::cout << "\n=== Picking candidate, Step = " << StepNo
				<< "\n" "Ready queue:";
		for (auto &C : RQ)
			std::cout << ' ' << C.SU->NodeNum << "(P" << C.Priority << ')';
		std::cout << std::endl;
